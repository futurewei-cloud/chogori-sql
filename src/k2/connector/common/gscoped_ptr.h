...
#include "common/util/template_util.h"
#include "common/type/type_traits.h"
#include "move.h"
...
//
// gscoped_ptr<int, yb::FreeDeleter> foo_ptr(
...
// types.
template <class T, class D = yb::DefaultDeleter<T> >
class gscoped_ptr {
  MOVE_ONLY_TYPE_FOR_CPP_03(gscoped_ptr, RValue)

  COMPILE_ASSERT(yb::internal::IsNotRefCounted<T>::value,
 ...
 private:
  typedef yb::internal::gscoped_ptr_impl<element_type, deleter_type>
 ...
 private:
  ...
  yb::internal::gscoped_ptr_impl<element_type, deleter_type> impl_;
...
// DEPRECATED: Use gscoped_ptr<C, yb::FreeDeleter> instead.
//
// gscoped_ptr_malloc<> is similar to gscoped_ptr<>, but it accepts a
// second template argument, the functor used to free the object.

template<class C, class FreeProc = yb::FreeDeleter>
