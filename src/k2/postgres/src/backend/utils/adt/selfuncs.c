...
patternsel(PG_FUNCTION_ARGS, Pattern_Type ptype, bool negate)
{
	...
	{
		case TEXTOID:
			opfamily = IsK2PgEnabled() ? TEXT_LSM_FAM_OID : TEXT_BTREE_FAM_OID;
			break;
		case BPCHAROID:
			opfamily = IsK2PgEnabled() ? BPCHAR_LSM_FAM_OID : BPCHAR_BTREE_FAM_OID;
			break;
		case NAMEOID:
			opfamily = IsK2PgEnabled() ? NAME_LSM_FAM_OID : NAME_BTREE_FAM_OID;
			break;
		case BYTEAOID:
			opfamily = IsK2PgEnabled() ? BYTEA_LSM_FAM_OID : BYTEA_BTREE_FAM_OID;
			break;
		default:
			ReleaseVariableStats(vardata);
			return result;
