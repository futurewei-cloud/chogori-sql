...
-- Index for SELECTing k2pgtid of the same airport type using RANGE key.
...
--
-- SELECT using a hash index, "airport_type_hash_idx", to collect k2pgtids.
--

-- The following select use batches of k2pgtid that are selected from "airport_type_hash_idx"
EXPLAIN (COSTS OFF) SELECT * FROM airports WHERE type = 'large_airport' AND iso_country > '0';

-- The following select use one k2pgtid at a time to query data.
...
--
-- SELECT using a range index, "airport_type_range_idx", to collet k2pgtids.
--

-- The following select use batches of k2pgtid that are selected from "airport_type_range_idx"
